          ...
          <div class="row">
           ...
           <div class="box">
                <div class="box-header">
                  <h3 class="box-title">Campaign Summary (Direct Leads)</h3>
                </div><!-- /.box-header -->
               <?php
...
<br/> <hr/> <br/>
  ...
  $selectEnq="SELECT b.ncfor  As Campaign, b.ncflag as CType, b.freshness as freshness,
    (Select COUNT(m.lnode) FROM `enquiry` m WHERE ".$stme." m.status=1 AND m.lnode=b.ncid AND m.`userid`!=0 AND b.ncflag!=1 AND (m.adate>='$fdate' AND m.adate<='$tdate') GROUP BY b.ncid) As  Total_Calls,
    (Select count(a.oid) FROM  `order` a, `enquiry` m  WHERE ".$stme."a.eid=m.eid AND m.lnode=b.ncid AND b.ncflag!=1 AND a.status=1  AND m.`userid`!=0 AND a.tqty!=0  AND (a.date>='$fdate' AND a.date<='$tdate') ) AS Total_Orders, 
    round((((Select Total_Orders)/((Select Total_Calls)))*100),2) AS Conversion_Rate,
    (Select SUM(a.gtotal) FROM  `order` a, `enquiry` m  WHERE ".$stme."a.eid=m.eid AND m.lnode=b.ncid AND b.ncflag!=1 AND a.status=1   AND m.`userid`!=0 AND (a.date>='$fdate' AND a.date<='$tdate')) AS Order_Value From `number_config` b WHERE b.ncstatus=1 GROUP BY b.ncid  HAVING (Total_Calls>=1 OR Total_Orders>=1) ORDER BY Order_Value DESC";
  ...
  ?>
   ...
   $selectEnqT="SELECT  
    (Select COUNT(m.lnode) FROM `enquiry` m ,`number_config` b WHERE ".$stme." b.ncstatus=1 AND m.status=1 AND m.lnode=b.ncid  AND m.`userid`!=0 AND b.ncflag!=1 AND (m.adate>='$fdate' AND m.adate<='$tdate')) As  Total_Calls,
    (Select count(a.oid) FROM  `order` a, `enquiry` m,`number_config` b WHERE".$stme." (b.ncstatus=1 AND a.eid=m.eid) AND m.lnode=b.ncid AND a.status=1 AND (a.date>='$fdate' AND a.date<='$tdate')  AND a.tqty!=0 AND b.ncflag!=1 ) AS Total_Orders, 
    round((((Select Total_Orders)/((Select Total_Calls)))*100),2) AS Conversion_Rate,
    (Select SUM(a.gtotal) FROM  `order` a, `enquiry` m,`number_config` b WHERE ".$stme." b.ncstatus=1 AND b.ncflag!=1 AND a.eid=m.eid AND m.lnode=b.ncid AND a.status=1  AND (a.date>='$fdate' AND a.date<='$tdate')) AS Order_Value";
