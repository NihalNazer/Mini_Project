...
{
	echo"<script type=\"text/javascript\">
 window.location.assign(\"edit_order.php\")
</script>";	
}
 ?>
          <?php 
...
if($item!='' && $qty!='')
	{ 
		...
		if(mysqli_num_rows($queryResult)>=1)
			...
			if($bulk==1)
{
$offerPrice1=$joinQryResultRow1["rprice"];
}
else
{
	$offerPrice1=$joinQryResultRow1["oprice"];
}
					
			$product1=$joinQryResultRow1["Product"];		
			/*************CALCULATIONS*************/
			$grandTotal=$offerPrice1*$qty;
			/***************************SELECT FROM ORDER****************************/
	...
	/***********************calculation************************/
			...
			$ordGrandTot2=$ordGrandTot1+$grandTotal;
				...
				$insertQuery="INSERT INTO `order_split` (`Osid`, `Orderid`, `Ccode`, `Citem`,  `Uprice`, `Qty`, `Gtotal`, `Remark`, `Status`) VALUES (NULL, '$orderId', '$item','$product1',  '$offerPrice1','$qty', '$grandTotal', '$remark', '1')";
	//echo $insertQuery;
...
if($ebulk==1)
{
$offerPrice=$joinQryResultRow2["rprice"];
}
else
{
...
/******************displaying calculated values*********************/
	...
	/*****************select values from corresponding order*******************/
		...
		/*****************update order_split******************/
		$updateQuery="UPDATE `order_split` SET `Ccode` =  '$edtCode',`Citem` =  '$edtItem',`Uprice` = '$offerPrice',`Qty` =  '$edtQty',`Gtotal` = '$edtGAmount',`Remark` =  '$edtRemark' WHERE `Osid` =$hid";
...
?>
  ...
  <div class="box-body">
	...
	$insertOrder="INSERT INTO `order` (`oid`,`date`,`oTime`,`customerid`,`eid`,`tme`,`status`,`oFlag`,`oallotment`,`invtype`) VALUES(NULL,'$CurrentDate','$CurrentTime','$custumer','$refid','$user','1','1','0','1')";
...
<form role="form" method="post" >
     ...
     <?php
	  ...
	  ?>
               ...
               <div class="form-group">
               <label for="qty">Quantity<span style="color:#FF0000">*</span></label>
...
<?php 
	if(@$ebulk==1)
	{
		?>
          ...
          <input id="ebulk"  name="chkboxeBulk" type="checkbox" value="1" checked="checked" />      
        <?php
	}
	else
	{
		?>
         <input id="ebulk" name="chkboxeBulk" type="checkbox" value="1" />    
        <?php		
	}
	?>    
     Special Offer 
...
else
{
	...
	if(@$bulk==1)
	{
		?>
          ...
          <input id="bulk"  name="chkboxBulk" type="checkbox" value="1" checked="checked" />      
        <?php
	}
	else
	{
		?>
         <input id="bulk" name="chkboxBulk" type="checkbox" value="1" />    
        <?php		
	}
	?>    
     Special Offer 
