...
while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
	...
	if($firstRow) { $firstRow = false; }
		...
		$selecta="select `eid` from `enquiry` where `sid`='$data[0]' and `vnum`='$tennumber' and `vctime`='$data[2]' AND `status`='1' AND `fresh`='1' ";
		...
		$selectb="select `eid` from `enquiry` where `vnum`='$tennumber' AND `status`='1' AND `flag`='0' AND `fresh`='1' ";
