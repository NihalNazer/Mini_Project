...
{
	$query="select `customerid` from `customer` where ((`contactNum1`='$contactNumber1' && `Country_code1`='$ccode1') || (`contactNum2`='$contactNumber2' && `contactNum2`!=0) || (`contactNum2`='$contactNumber1' && `contactNum2`!=0) || (`contactNum1`='$contactNumber2')) and `Status`=1 ORDER BY `customerid` DESC LIMIT 0,1";
//	echo $query;
	$queryResult=mysqli_query($con, $query);
	if(mysqli_num_rows($queryResult)>=1)
	{
		$newrowqueryResult=mysqli_fetch_array($queryResult);
		$newcustumercode=$newrowqueryResult["customerid"];
		
	$newupdateEnq="UPDATE `enquiry` SET `customerid`='$newcustumercode' WHERE `eid`='$refid'";
	mysqli_query($con, $newupdateEnq);

	  echo"<div class=\"alert alert-danger alert-dismissable\">
                    <button type=\"button\" class=\"close\" data-dismiss=\"alert\" aria-hidden=\"true\">&times;</button>
                    <h4><i class=\"icon fa fa-ban\"></i> Alert!</h4>
                    CUSTOMER: $name ALREADY EXISTING IN THIS MOBILE NUMBER.!
                  </div>";
  }//exist check
  else
  {		
...
//	echo $insertQuery;
	...
	$aquery="select `customerid` from `customer` where ((`Country_code1`='$ccode1' &&`contactNum1`='$contactNumber1') || (`contactNum2`='$contactNumber2' && `contactNum2`!=0) || (`contactNum2`='$contactNumber1' && `contactNum2`!=0) || (`contactNum1`='$contactNumber2')) and `Status`=1 ORDER BY `customerid` DESC LIMIT 0,1";
//	echo $query;
...
if($eName!='' && $eDistrict!='' && $eHouse!='' && $ePlace!='' && $ePlace!='' && $ePost!=''&& $edtCountry1!='' && $edtContact1!='')
	{
	$querySelect="SELECT `customerid` FROM  `customer` WHERE ((`Country_code1`='$edtCountry1' and `contactNum1` ='$edtContact1')  OR ( `Country_code2`='$edtCountry2' and `contactNum2` ='$edtContact2' and `contactNum2` !='0') OR (`contactNum2`='$edtContact1' && `contactNum2`!=0) OR (`contactNum1`='$edtContact2')) and `customerid`!='$hid'";
		...
		if(mysqli_num_rows($resultCustomer)>=1)
		{
			 echo"<div class=\"alert alert-danger alert-dismissable\">
                    <button type=\"button\" class=\"close\" data-dismiss=\"alert\" aria-hidden=\"true\">&times;</button>
                    <h4><i class=\"icon fa fa-ban\"></i> Alert!</h4>
                    CUSTOMER: $eName  ALREADY EXISTING IN THIS PHONE NUMBER.! <br/>
