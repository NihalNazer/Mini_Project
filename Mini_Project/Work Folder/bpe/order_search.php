...
<hr/>
	  ...
	  if($searchToDate!='')
	  {
		  $dsearch=" a.date='$searchToDate' AND ";
	  }
	  else
	  {
		  $dsearch=' ';
	  }
	$selectBase="select a.oid, a.eid, a.date, b.name, a. tqty, a.gtotal, g.OsName, a.tme, g.OsId, e.Pincode, e.Officename from `order` a, `customer` b, `district` c, `pin` e, `order_status` g, `user` u where ".$dsearch." (a.`oid`='$searchValue' or b.`contactNum1` LIKE '$searchValue' or b.`contactNum2` LIKE '$searchValue') AND a.customerid=b.customerid and  b.districtid=c.Did and b.post=e.Pid and u.Uid=a.tme and g.OsId=a.oFlag and a.`status`='1' and a.`oFlag`>='2' ORDER BY a.oid DESC";
