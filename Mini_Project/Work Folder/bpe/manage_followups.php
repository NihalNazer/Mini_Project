...
{
	 $refid=$_POST["refid"];
	 mysqli_query($con, "update `enquiry` set `flag`='2' WHERE `eid`='$refid'");
...
<form role="form" method="post" >
	...
	$selectResponseCat="select * from `response_category` where `rtid`='$editCatId' and `rtstatus`='1' order by `name` ASC";
	...
	?>
      ...
      <?php 
	$selectOtherCatg="select * from `response_category` where `rtid`!='$editCatId' and `rtstatus`='1' order by `name` ASC";
...
if(isset($refid))
{
	...
	$editId=$refid;
	//echo "you are editting";
	$editSelect="SELECT * FROM `enquiry` WHERE `eid`='$editId'";
    //echo $editSelect;
	$result=mysqli_query($con, $editSelect);
	while($editRow=mysqli_fetch_array($result))
	{
...
<?php 
				 $enq=$editRow["eid"];
				$selectAction="select a.rid From response a, response_category b where a.rtype=b.rtid AND a.rstatus=1 And b.rtstatus=1 and b.rflag=0 AND a.Enquiry_id=$enq";
				$resultAction=mysqli_query($con, $selectAction);
				//while ($rowAction=mysqli_fetch_array($resultAction)>=1)
				if ($rowAction=mysqli_num_rows($resultAction)>=1)
				{
				 ?>
              ...
              <div class="box-header">
				...
				?>
                 ...
                 <?php 
				 $enq=$editRow["eid"];
				$selectConverted="select a.rid From response a, response_category b where a.rtype=b.rtid AND a.rstatus=1 And b.rtstatus=1 and b.rflag=0 AND a.Enquiry_id=$enq AND b.rtid=11";
				$resultConverted=mysqli_query($con, $selectConverted);
				//while ($rowConverted=mysqli_fetch_array($resultConverted)>=1)
				if ($rowConverted=mysqli_num_rows($resultConverted)>=1)
				{
				 ?>
                
                   <td>
                  <form action="customers.php" method="post">
                   <input type="hidden" name="refid" value="<?php echo $editRow["eid"]; ?>" /> 
                  <div class="input-group input-group-lg">
                  <input name="pinc" class="form-control" type="number" min="670001" max="695615" placeholder="PIN" required/>
                  
                  <span class="input-group-btn">
                  <button type="submit" name="btnRef"  class="btn btn-success"><i class="fa fa-shopping-cart"></i> Make an Order </button>
                  </span>
                 </div>
                  </form>                 
                  </td>
				...
				?>
                  ...
                  <td>&nbsp;
                   ...
                   <?php 
				 $enq=$editRow["eid"];
				$selectDrop="select a.*, b.* From response a, response_category b where a.rtype=b.rtid AND a.rstatus=1 And b.rtstatus=1 and b.rflag=0 AND a.Enquiry_id=$enq AND b.rtid!=11";
				$resultDrop=mysqli_query($con, $selectDrop);
				//while ($rowDrop=mysqli_fetch_array($resultDrop)>=1)
				if ($rowDrop=mysqli_num_rows($resultDrop)>=1)
				{
				 ?>
                   <td>
                  <form action="" method="post">
                  <input type="hidden" name="refid" value="<?php echo $editRow["eid"]; ?>" />  
                  <button type="submit" name="drop"  onClick="return confirm('Are you sure you want to drop this?');" class="btn btn-danger"><i class="fa fa-trash-o"></i> Drop This Lead</button>
                  </form>                 
                  </td>
                  <?php
				}
