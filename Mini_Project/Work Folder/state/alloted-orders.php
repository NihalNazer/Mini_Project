...
$ono=$ovalues=0;
  ...
  $selectEnq="Select count(a.oid) as 'Nos',  SUM(a.gtotal) as 'values', os.OsName FROM  `order` a, `user` b, `order_status` os, `district` d WHERE a.oallotment=b.Uid and ".$stme." d.Did=b.Dist and os.OsId=a.oFlag and a.status=1 AND a.tqty!=0 AND (a.date>='$fdate' AND a.date<='$tdate') GROUP BY a.oFlag  HAVING (Nos>=1) ORDER BY 'values' DESC ";
