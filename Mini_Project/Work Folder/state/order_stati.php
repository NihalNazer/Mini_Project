	...
	if($tdate!='')
{
					...
					$n=1;
	  $i = 0;
$selectLead="SELECT a.*, b.*, c.Officename, c.Pincode, e.*, u.Name FROM  `order` a, `customer` b, `pin` c,  `district` e, `user` u  WHERE a.`oid`='$tdate' AND  u.Uid=a.tme AND a.customerid= b.customerid AND b.post=c.Pid AND b.districtid=e.Did AND  a.status=1 AND a.tqty!=0 AND (a.oFlag='2' OR a.oFlag='8' OR a.oFlag='9') AND  (a.invtype=1 OR a.invtype=2)";
        ...
        <tr class="contentsrow">
        	<td>
             <address>
               ...
               <strong> 
                <?php 
                $dist=$rowLead["districtid"]; 
                $cagent=$rowLead["oallotment"];
                ?>
...
$selectTme="SELECT Uid, Name FROM  `user` WHERE `Uid`='$cagent' and `Status`=1 AND `Categoryid`='8'  ORDER BY Name ASC";
...
?>
 <?php 
$selectTme="SELECT Uid, Name FROM  `user` WHERE  `Status`=1 AND `Categoryid`='8' AND `Dist`='$dist' ORDER BY Name ASC";
$resultTme=mysqli_query($con, $selectTme);
while($rowTme=mysqli_fetch_array($resultTme))
{
?>
