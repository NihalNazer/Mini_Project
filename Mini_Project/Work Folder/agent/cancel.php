   ...
   <?php  bdm_nav($con); ?>
      ...
      <div class="content-wrapper">
        ...
        <section class="content">
           <?php
 if(isset($_POST["btnRef"])|| isset($_POST["btnSave"]))
 {
	 $refid=$_POST["refid"];
 }
else
{
	echo"<script type=\"text/javascript\">
 window.location.assign(\"order_stati.php\")
</script>";	
}
 ?>
          <?php 
...
if($rr!='')
{
$updateStatus="Update `order` SET  `oFlag`='12' WHERE `oid`='$Ordrid'";
	...
	$insertOrdr="INSERT INTO `order_status_update` 
		...
		VALUES (NULL, '$Ordrid', '12', '$user', '$CurrentDate', '$CurrentTime', '1', '1','$rrmark')";
		...
		echo"<div class=\"alert alert-success alert-dismissable\">
                   ...
                   Cancelation Entered Successfully ...
                  </div>";
    ...
    <!-- left column -->
            <div class="col-md-6">
              ...
              <div class="box box-primary">
                <div class="box-header">
               <i class="fa fa-shopping-cart"></i>
                  <h3 class="box-title">Reason for Cancelation</h3>
                </div><!-- /.box-header -->
 <!----------------------------------------------ADD NEW---------------------------------------------------->
                <!-- form start -->
	...
	$selectProduct="select a.* from `cancel_reason` a where  a.`CR_id`='$editId' and  a.CR_status='1' order by a.CR_name ASC";
	$resultQuery=mysqli_query($con, $selectProduct);
	while($resultRow=mysqli_fetch_array($resultQuery))
	{
	?>
    <option selected="selected" value="<?php echo $resultRow["CR_name"];?>"><?php echo $resultRow["CR_name"];?></option>
    <?php
	}
	?>
	...
	$selectOtherProducts="select a.* from `cancel_reason` a where  a.`CR_id`!='$editId' and  a.CR_status='1' order by a.CR_name ASC";
	$resultProducts=mysqli_query($con, $selectOtherProducts);
	while($row=mysqli_fetch_array($resultProducts))
	{
	?>
    <option value="<?php echo $row["CR_name"];?>"><?php echo $row["CR_name"];?></option>
    <?php
	}
	 ?>
