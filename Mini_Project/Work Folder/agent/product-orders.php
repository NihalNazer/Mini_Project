...
<br/> <hr/> <br/>
	 ...
	 $selectEnq="SELECT b.Ccode AS Item_Code, b.Citem AS Item, SUM(b.Qty) AS Orders, SUM(b.Gtotal) AS Value FROM  `order` a, `order_split` b,  `user` f,  `district` d WHERE  a.oid= b.Orderid AND a.oallotment = f.Uid AND f.Uid=$user AND d.Did=f.Dist AND (a.date>='$fdate' AND a.date<='$tdate') AND b.Gtotal!=0  AND b.Status=1 GROUP BY b.Ccode ORDER BY `Value` DESC";
	...
	?>
	 ...
	 $selectEnqT="SELECT b.Ccode AS Item_Code, b.Citem AS Item, SUM(b.Qty) AS Orders, SUM(b.Gtotal) AS Value FROM  `order` a, `order_split`  b,  `user` f,  `district` d  WHERE a.oid= b.Orderid  AND a.oallotment = f.Uid AND f.Uid=$user AND d.Did=f.Dist AND (a.date>='$fdate' AND a.date<='$tdate') AND b.Gtotal!=0  AND b.Status=1";
